...
namespace Fixera
{
    public class DefaultFakeProvider : IFakeProvider
    {
        ...
        public TFake GetTransient<TFake>() where TFake : class
        {
            return _factory.Create<TFake>();
        }

        public TFake GetSingleton<TFake>() where TFake : class
        {
            return (TFake) _singletons.GetOrAdd(typeof(TFake), _ => _factory.Create<TFake>());
        }

        public bool TryAddSingleton<TFake>(TFake instance) where TFake : class
        {
